...
*/
  ...
  it("navigateSearchAndDelete", async () => {
    ...
    await page.type('[class=el-input__inner]', username);
    delay(200000);
console.log('%cgaCreateDeleteCourse-10.spec.js line:532 2222', 'color: #007acc;', 2222);
    //******Act******* */

    //check the link in table to much the created link
    const content4 = await page.$eval('tr td:nth-child(3) div a', e => e.textContent);
    chai.expect(content4).to.equal(regLink);
    console.log('%cgaCreateDeleteCourse-10.spec.js line:538 333', 'color: #007acc;', 333);
    await page.evaluate(() => {
      document.getElementsByClassName('el-icon-delete')[0].click()
    });
    console.log('%cgaCreateDeleteCourse-10.spec.js line:542 44', 'color: #007acc;', 44);
